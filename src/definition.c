...
void	draw_def(t_fdf *map)
{
	...
	if (def)
	{
		...
		if (def->nexty)
		{
			...
			mlx_string_put(map->win.mlx, map->win.win, WIDTH - WIDTHD - 30, 30, CWHI, ft_itoa(map->LOW_RANGE));
		}
		if (def->nexty->nextx)
...
t_coord		*init_def_coord(t_fdf *map)
{
	...
	begdef->nextx->nexty = creat_coord(defdot(WIDTHD, HEIGHTD, map->HIGH_RANGE));
